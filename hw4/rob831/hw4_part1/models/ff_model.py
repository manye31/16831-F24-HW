...
class FFModel(nn.Module, BaseModel):
    ...
    def forward(
        ...
        obs_normalized = normalize(obs_unnormalized, obs_mean, obs_std) # DONE TODO(Q1)
        acs_normalized = normalize(acs_unnormalized, acs_mean, acs_std) # DONE TODO(Q1)

        # predicted change in obs
        concatenated_input = torch.cat([obs_normalized, acs_normalized], dim=1)

        # DONE TODO(Q1) compute delta_pred_normalized and next_obs_pred
        # Hint: as described in the PDF, the output of the network is the
        # *normalized change* in state, i.e. normalized(s_t+1 - s_t).
        delta_pred_normalized = self.delta_network(concatenated_input) # DONE TODO(Q1)
        next_obs_pred = obs_unnormalized + unnormalize(delta_pred_normalized, delta_mean, delta_std) # DONE TODO(Q1)
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        prediction = self.forward(
            ...
            self.delta_mean,
            self.delta_std
        )[0] # DONE TODO(Q1) get the predicted next-states (s_t+1) as a numpy array
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        self.update_statistics(**data_statistics)
        # DONE TODO(Q1) compute the normalized target for the model.
        ...
        loss = self.loss(preds, target) # DONE TODO(Q1) compute the loss
        # Hint: `self(...)` returns a tuple, but you only need to use one of the
        # outputs.
